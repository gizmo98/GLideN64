	...
	public:
		DitherFilter(const opengl::GLInfo & _glinfo)
		{
			m_part =
				...
				"    vec2 fuv = fract( st );							\n"
				"														\n"
				"    vec4 a = texture( uTex0, (iuv+vec2(0.5,0.5))/res );\n"
				"    vec4 b = texture( uTex0, (iuv+vec2(1.5,0.5))/res );\n"
				"    vec4 c = texture( uTex0, (iuv+vec2(0.5,1.5))/res );\n"
				"    vec4 d = texture( uTex0, (iuv+vec2(1.5,1.5))/res );\n"
				"														\n"
