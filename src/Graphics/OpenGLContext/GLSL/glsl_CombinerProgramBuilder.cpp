		...
		if (!m_glinfo.isGLES2) {
			...
			if (g_textureConvert.useTextureFiltering()) {
				...
				shaderPart +=
					...
					"{																	\\\n"
					"  uv *= res; 														\\\n"
					"  return (saturate(fract(uv) / saturate(fwidth(uv))) + floor(uv) - 0.5) / res;\\\n"
					"}																	\\\n"
					"vec4 pointSamplingAA(in sampler2D tex, in vec2 uv){				\\\n"
					...
					"  } else {															\\\n"
					"    if (uTextureFilterMode == 0) name = pointSamplingAA(tex, texCoord);\\\n"
